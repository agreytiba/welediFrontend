        ...
        return (
            <div className="website__Settings">
                ...
                <Collapse in={this.state.isSuccesShowed}>
                    <Alert
                        onClose={() => {
                            this.setState((prevState) => ({ isSuccesShowed: !prevState.isSuccesShowed }));
                        }}>
                        New page added succefully!
                    </Alert>
                </Collapse>
                {/* Form */}
                <form>
                    <TextField
                        ...
                        variant="outlined"
                    />
                    <ReactQuill style={{ marginTop: '10px' }} value={this.state.text} onChange={this.handleTextChange} />
                    <div className="settings__subtitle">Current Pages</div>
                    {/* Current Pages */}
                    <div className="settings__pages">
                        {/* no pages show text  */}
                        {this.state.pages == null ? (
                            <div className="settings__noPages">No pages found !</div>
                        ) : (
                            <ul>
                                {this.state.pages.map((value, index) => {
                                    return (
                                        <li key={index}>
                                            ...
                                            <img onClick={() => this.removePageHandler(value.id)} src={RemoveImage} alt="remove" />
                                        </li>
                                    );
                                })}
                            </ul>
